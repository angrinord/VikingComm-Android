...
//        }
    ...
    private void enableAndRefreshInCallActions() {
        ...
        mConference.setEnabled(LinphoneManager.getLc().getCallsNb() > 1 && LinphoneManager.getLc().getCallsNb() > confsize && !LinphoneManager.getLc().soundResourcesLocked());

        mAddCall.setEnabled(LinphoneManager.getLc().getCallsNb() < LinphoneManager.getLc().getMaxCalls() && !LinphoneManager.getLc().soundResourcesLocked());
        mOptions.setEnabled(!getResources().getBoolean(R.bool.disable_options_in_call) && (mAddCall.isEnabled() || mTransfer.isEnabled()));

        Call currentCall = LinphoneManager.getLc().getCurrentCall();

        mRecordCall.setEnabled(!LinphoneManager.getLc().soundResourcesLocked() && currentCall != null && currentCall.getCurrentParams().getRecordFile() != null);
        ...
        mVideo.setEnabled(currentCall != null && LinphonePreferences.instance().isVideoEnabled() && !currentCall.mediaInProgress());
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.video) {
            ...
            //                            + (camera == PackageManager.PERMISSION_GRANTED ? "granted"
            // : "denied"));
            //
    ...
    private void showAcceptCallUpdateDialog() {
        ...
        Drawable d = new ColorDrawable(ContextCompat.getColor(this, R.color.new_grey_color));
        d.setAlpha(200);
...
//                        }
        ...
        cancel.setOnClickListener(
                new OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        if (CallActivity.isInstantiated()) {
    ...
    private void displayPausedCalls(final Call call, int index) {
        ...
        if (call == null) {
            callView = (LinearLayout) mInflater.inflate(R.layout.conference_paused_row, mContainer, false);
            ...
            callView = (LinearLayout) mInflater.inflate(R.layout.call_inactive_row, mContainer, false);
            callView.setId(index + 1);
    ...
    private void refreshCallList() {
        ...
        List<Call> pausedCalls = LinphoneUtils.getCallsInState(LinphoneManager.getLc(), Collections.singletonList(State.PausedByRemote));
        ...
        if (LinphoneManager.getLc().getCurrentCall() != null) {
            ...
            if (isVideoEnabled(LinphoneManager.getLc().getCurrentCall()) && !mIsConferenceRunning && pausedCalls.size() == 0) {
                displayVideoCall(false);
            } else {
                displayAudioCall();
            }
    ...
    private void displayConferenceParticipant(int index, final Call call) {
        LinearLayout confView = (LinearLayout) mInflater.inflate(R.layout.conf_call_control_row, mContainer, false);
    ...
    private void displayConferenceHeader() {
        ...
        RelativeLayout headerConference = (RelativeLayout) mInflater.inflate(R.layout.conference_header, mContainer, false);
    ...
    private void displayMediaStats(
        ...
        if (stats != null) {
            ...
            if (media != null) {
                ...
                formatText(codec, getString(R.string.call_stats_codec), mime + " / " + (media.getClockRate() / 1000) + "kHz");
            }
            if (mime != null) {
            ...
            formatText(dl, getString(R.string.call_stats_download), String.valueOf((int) stats.getDownloadBandwidth()) + " kbits/s");
            formatText(ul, getString(R.string.call_stats_upload), String.valueOf((int) stats.getUploadBandwidth()) + " kbits/s");
            if (isVideo) {
                formatText(edl, getString(R.string.call_stats_estimated_download), String.valueOf(stats.getEstimatedDownloadBandwidth()) + " kbits/s");
            }
            formatText(ice, getString(R.string.call_stats_ice), stats.getIceState().toString());
            formatText(
                    ip, getString(R.string.call_stats_ip), (stats.getIpFamilyOfRemote() == AddressFamily.Inet6) ? "IpV6" : (stats.getIpFamilyOfRemote() == AddressFamily.Inet) ? "IpV4" : "Unknown");
            formatText(senderLossRate, getString(R.string.call_stats_sender_loss_rate), new DecimalFormat("##.##").format(stats.getSenderLossRate()) + "%");
            formatText(receiverLossRate, getString(R.string.call_stats_receiver_loss_rate), new DecimalFormat("##.##").format(stats.getReceiverLossRate()) + "%");
            if (isVideo) {
                formatText(videoResolutionSent, getString(R.string.call_stats_video_resolution_sent), "\u2191 " + params.getSentVideoDefinition() != null ? params.getSentVideoDefinition().getName() : "");
                formatText(videoResolutionReceived, getString(R.string.call_stats_video_resolution_received), "\u2193 " + params.getReceivedVideoDefinition() != null ? params.getReceivedVideoDefinition().getName() : "");
                formatText(videoFpsSent, getString(R.string.call_stats_video_fps_sent), "\u2191 " + params.getSentFramerate());
                formatText(videoFpsReceived, getString(R.string.call_stats_video_fps_received), "\u2193 " + params.getReceivedFramerate());
            } else {
                formatText(jitterBuffer, getString(R.string.call_stats_jitter_buffer), new DecimalFormat("##.##").format(stats.getJitterBufferSizeMs()) + " ms");
            }
        } else {
    ...
    private void initCallStatsRefresher(final Call call, final View view) {
        ...
        CallListenerStub mCallListener = new CallListenerStub() {
                                ...
                                new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        synchronized (LinphoneManager.getLc()) {
                                            if (LinphoneActivity.isInstantiated() && call.getState() != Call.State.Released) {
