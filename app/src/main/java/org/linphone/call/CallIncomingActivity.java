...
import org.linphone.views.CallIncomingSnoozeButton;
...
public class CallIncomingActivity extends LinphoneGenericActivity {
    ...
    private CallIncomingSnoozeButton mSnooze;
    private Call mCall;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSnooze = findViewById(R.id.snooze_button);
        mSnooze.setSpacer(findViewById(R.id.spacer));
        mSnooze.setScrollHeight(findViewById(R.id.snooze_unlock).getHeight());
        mAcceptIcon = findViewById(R.id.acceptIcon);
        lookupCurrentCall();

        if (LinphonePreferences.instance() != null && mCall != null && mCall.getRemoteParams() != null && LinphonePreferences.instance().shouldAutomaticallyAcceptVideoRequests() && mCall.getRemoteParams().videoEnabled()) {
        ...
        mAccept.setListener(new CallIncomingButtonListener() {
                    @Override
                    public void onAction() {
                        answer();
                    }
                });
        mDecline.setListener(new CallIncomingButtonListener() {
                    @Override
                    public void onAction() {
                        decline();
                    }
                });
        mSnooze.setListener(new CallIncomingButtonListener() {
            @Override
            public void onAction() {
                snooze();
            }
    ...
    private void lookupCurrentCall() {
        if (LinphoneManager.getLcIfManagerNotDestroyedOrNull() != null) {
            for (Call call : LinphoneManager.getLc().getCalls()) {
                if (State.IncomingReceived == call.getState() || State.IncomingEarlyMedia == call.getState()) {
                    mCall = call;
                    break;
                }
            }
    ...
    private void decline() {
        if (mAlreadyAcceptedOrDeniedCall) { return; }
    ...
    private void snooze() {
        ...
        LinphoneManager.getLc().terminateCall(mCall);
        ((LinphoneApp)getApplication()).startSnooze();
    }
    ...
    private void answer() {
        ...
        if (!LinphoneManager.getInstance().acceptCall(mCall)) {
            ...
            if (!LinphoneActivity.isInstantiated()) { return; }
    ...
    private void checkAndRequestCallPermissions() {
        ...
        //        int camera =
        //                getPackageManager().checkPermission(Manifest.permission.CAMERA,
        // getPackageName());
        //        Log.i(
