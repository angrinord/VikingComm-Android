...
public class LinphoneActivity extends LinphoneGenericActivity
    ...
    private RelativeLayout mContacts, mHistory, mDialer, mChat, mSchedule;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //
        if (LinphoneService.isReady() && LinphoneService.instance().isBlocked()) {
            LinphoneService.instance().setBlocked(false);
            finishAndRemoveTask();
            return;
        }
    ...
    private void initButtons() {
        ...
        mScheduleSelected = findViewById(R.id.schedule_select);
    ...
    public void displayContactsForEditing(String sipAddress) {
        Bundle extras = new Bundle();
    ...
    public void displayContactsForEditing(String sipAddress, String displayName) {
        Bundle extras = new Bundle();
        extras.putBoolean("EditOnClick", true);
        extras.putString("SipAddress", sipAddress);
        extras.putString("DisplayName", displayName);
    ...
    public void goToChatGroupInfos(
        ...
        Bundle extras = new Bundle();
        extras.putString("groupChatRoomAddress", address);
        extras.putBoolean("isEditable", isEditable);
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.history) {
            ...
            changeCurrentFragment(FragmentsAvailable.SCHEDULE_LIST, null);
            mScheduleSelected.setVisibility(View.VISIBLE);
    ...
    private void resetSelection() {
        ...
        mScheduleSelected.setVisibility(View.GONE);
...
//        extras.putString("SipUri", "");
    ...
    public void displayMissedCalls(final int missedCallsCount) {
        if (missedCallsCount > 0) {
            ...
            if (LinphoneManager.isInstantiated()) LinphoneManager.getLc().resetMissedCallsCount();
    ...
    public void resetClassicMenuLayoutAndGoBackToCallIfStillRunning() {
        ...
        if (LinphoneManager.isInstantiated() && LinphoneManager.getLc().getCallsNb() > 0) {
            Call call = LinphoneManager.getLc().getCalls()[0];
    ...
    public void showSnoozeIndicator(boolean status){
        mStatusFragment.showSnoozeIndicator(status);
    }
}
