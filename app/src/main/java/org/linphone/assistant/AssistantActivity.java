...
public class AssistantActivity extends ThemableActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getBooleanExtra("LinkPhoneNumber", false)) {
            ...
            mFirstFragment = getResources().getBoolean(R.bool.assistant_use_linphone_login_as_first_fragment) ? AssistantFragmentsEnum.LINPHONE_LOGIN : AssistantFragmentsEnum.LOGIN;
            if (mFirstFragment == AssistantFragmentsEnum.WELCOME) {
                mFirstFragment = getResources().getBoolean(R.bool.assistant_use_create_linphone_account_as_first_fragment) ? AssistantFragmentsEnum.CREATE_ACCOUNT : AssistantFragmentsEnum.LOGIN;
            }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.back) {
            hideKeyboard();
            if (mCurrentFragment == AssistantFragmentsEnum.LOGIN) {
                LinphonePreferences.instance().firstLaunchSuccessful();
                if (getResources().getBoolean(R.bool.assistant_cancel_move_to_back)) {
                    moveTaskToBack(true);
                } else {
    ...
    public void displayLoginGeneric(JSONObject json) {
        mFragment = new LoginFragment();
        try {
            Bundle args = new Bundle();
            args.putString("url", json.getString("url"));
            args.putString("num", json.getString("num"));
            try{
                args.putString("pass", json.getString("pass"));
                args.putBoolean("multi", false);
            }
            catch (JSONException e) {
                args.putString("passes", json.get("passes").toString());
